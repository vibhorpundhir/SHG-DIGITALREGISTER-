...
import { findMember, findSHG, findVillage, findGP, getMemberHistory, getMemberReport, getMemberAttendance, getMemberSavingsTimeline, getMemberLoanTimeline, formatMonthHindi } from "@/lib/store";
...
function MemberTalpatPage() {
  ...
  const attendance = useMemo(() => (member && shg ? getMemberAttendance(member.name, shg.name) : null), [member, shg]);
  const savingsTimeline = useMemo(() => (member && shg ? getMemberSavingsTimeline(member.name, shg.name) : []), [member, shg]);
  const loanTimeline = useMemo(() => (member && shg ? getMemberLoanTimeline(member.name, shg.name) : []), [member, shg]);
